...
function loadCommonComponents() {
    fetch("components/chatbot.html?" + Date.now()) // ← 캐시 방지
        .then(res => res.text())
        .then(html => {
            console.log("📦 챗봇 HTML 로드:", html.substring(0, 200)); // 디버깅
...
function initChatbotEventListeners() {
    ...
    // 새로 추가된 버튼들
    const historyBtn = document.querySelector(".header-icon-btn[title='히스토리']");
    const newChatBtn = document.querySelector(".header-icon-btn[title='새 대화']");
